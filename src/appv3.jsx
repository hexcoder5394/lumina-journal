...
import { 
  ...
  CircleDollarSign, TrendingUp, TrendingDown, ExternalLink
} from 'lucide-react';
...
export default function JournalApp() {
  ...
  const [habits, setHabits] = useState(() => {
    ...
    if (saved) {
      ...
      if (parsed.date === new Date().toISOString().split('T')[0]) return parsed.items;
    }
    return [
  ...
  const [transactions, setTransactions] = useState(() => {
    const saved = localStorage.getItem('lumina_finance');
  ...
  useEffect(() => { localStorage.setItem('lumina_finance', JSON.stringify(transactions)); }, [transactions]);
  ...
  const addTransaction = (type) => {
    if (!finAmount || !finLabel) return;
    const newTx = {
      id: Date.now(),
      amount: parseFloat(finAmount),
      label: finLabel.toUpperCase(),
  ...
  const getBalance = () => {
    return transactions.reduce((acc, curr) => curr.type === 'credit' ? acc + curr.amount : acc - curr.amount, 0);
  };
  ...
  return (
    <div className="h-screen w-full bg-cyber-dark bg-grid-pattern text-gray-300 font-body selection:bg-cyber-pink selection:text-white flex flex-col overflow-hidden">
      ...
      {/* Main Content Area */}
      <div className="flex-1 w-full px-6 py-4 overflow-hidden flex flex-col">
        ...
        {/* ================= DASHBOARD VIEW (FULL SCREEN GRID) ================= */}
        {view === 'dashboard' && (
          <div className="flex-1 grid grid-cols-1 md:grid-cols-4 grid-rows-[1fr_1.2fr_1fr] gap-4 animate-fadeIn overflow-hidden">
            ...
            {/* WIDGET 5: RESOURCE MONITOR (FINANCE) - NEW */}
            <div className="md:col-span-2 border border-white/10 bg-cyber-slate/30 p-6 flex flex-col relative overflow-hidden h-full">
              <div className="flex justify-between items-start mb-4 shrink-0">
                <div>
                   <h3 className="text-xs font-mono text-gray-500 flex items-center gap-2 mb-1"><CircleDollarSign className="w-3 h-3" /> // CREDITS_ALLOCATION</h3>
                   <div className={`text-3xl font-display font-bold ${getBalance() >= 0 ? 'text-white' : 'text-red-500'}`}>${getBalance().toFixed(2)}</div>
                </div>
                <button 
                  onClick={() => window.open('https://budget.infinityfree.me/?i=1', '_blank')}
                  className={`flex items-center gap-2 px-3 py-1.5 border border-white/20 rounded hover:bg-white/10 transition-all text-[10px] font-mono uppercase ${accentText}`}
                >
                  <ExternalLink className="w-3 h-3" /> Access_Mainframe
                </button>
              </div>

              {/* Transaction List */}
              <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 flex flex-col gap-2 mb-4">
                 {transactions.length === 0 && <div className="text-center text-[10px] text-gray-600 font-mono py-4">NO_RECENT_TRANSACTIONS</div>}
                 {transactions.map(tx => (
                   <div key={tx.id} className="flex justify-between items-center p-2 border border-white/5 bg-black/20 text-xs font-mono shrink-0">
                      <div className="flex items-center gap-2">
                        {tx.type === 'credit' ? <TrendingUp className="w-3 h-3 text-green-500" /> : <TrendingDown className="w-3 h-3 text-red-500" />}
                        <span className="text-gray-400">{tx.label}</span>
                      </div>
                      <div className="flex items-center gap-3">
                         <span className={tx.type === 'credit' ? 'text-green-500' : 'text-red-500'}>
                           {tx.type === 'credit' ? '+' : '-'}${tx.amount.toFixed(2)}
                         </span>
                         <button onClick={() => deleteTransaction(tx.id)} className="text-gray-700 hover:text-red-500"><X className="w-3 h-3" /></button>
                      </div>
                   </div>
                 ))}
              </div>
              ...
              <div className="grid grid-cols-4 gap-2 shrink-0">
                 <input type="text" placeholder="LABEL" value={finLabel} onChange={(e) => setFinLabel(e.target.value)} className="col-span-2 bg-black/40 border border-white/10 text-[10px] font-mono p-2 text-white focus:outline-none uppercase" />
                 <input type="number" placeholder="0.00" value={finAmount} onChange={(e) => setFinAmount(e.target.value)} className="bg-black/40 border border-white/10 text-[10px] font-mono p-2 text-white focus:outline-none" />
                 <div className="flex gap-1">
                    <button onClick={() => addTransaction('credit')} className="flex-1 bg-green-900/30 border border-green-500/30 hover:bg-green-500 hover:text-black text-green-500 transition-colors flex items-center justify-center"><Plus className="w-4 h-4" /></button>
                    <button onClick={() => addTransaction('debit')} className="flex-1 bg-red-900/30 border border-red-500/30 hover:bg-red-500 hover:text-black text-red-500 transition-colors flex items-center justify-center"><Minus className="w-4 h-4" /></button>
                 </div>
              </div>
            </div>
            ...
            {/* WIDGET 6: NET-MEMO (Full Width) */}
            <div className="md:col-span-4 border border-white/10 bg-cyber-slate/30 p-6 relative h-full flex flex-col">
              <h3 className="text-xs font-mono text-gray-500 mb-2 flex items-center gap-2 shrink-0">
              ...
              <textarea
                ...
                className="flex-1 w-full bg-transparent text-cyber-cyan font-mono text-sm focus:outline-none resize-none placeholder-gray-700 p-2 border-l border-white/5 focus:border-cyber-cyan/50 transition-colors"
              />
              <div className="absolute bottom-4 right-4 text-[10px] text-gray-600 uppercase">
                SYSTEM_AUTO_SAVE_ACTIVE
              </div>
            </div>
        ...
        {/* ================= WRITE VIEW, ENTRIES, STATS (UNCHANGED) ================= */}
        {view === 'write' && (
          <div className="flex-1 border border-white/10 bg-cyber-slate/50 p-6 md:p-8 animate-fadeIn relative flex flex-col overflow-hidden">
            ...
            <div className="flex flex-wrap justify-between items-end gap-4 border-b border-white/10 pb-6 shrink-0">
            ...
            <textarea value={currentEntry.content} onChange={(e) => setCurrentEntry({...currentEntry, content: e.target.value})} placeholder="INITIATE LOG SEQUENCE..." className="flex-1 w-full bg-black/20 text-lg leading-relaxed p-4 focus:outline-none border-l-2 border-transparent focus:border-l-cyber-cyan transition-all text-gray-200 resize-none font-mono mt-6 mb-4" />
