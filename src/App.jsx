...
export default function JournalApp() {
  ...
  const addCommandLink = () => {
    ...
    setCommandLinks(updated);
    setNewLink({ title: '', url: '' });
    if(user) saveDashboard(user.uid, memo, habits, updated, subscriptions);
  };

  const removeCommandLink = (id) => {
    const updated = commandLinks.filter(l => l.id !== id);
    setCommandLinks(updated);
    if(user) saveDashboard(user.uid, memo, habits, updated, subscriptions);
  };
  ...
  const addSubscription = () => {
    ...
    const updated = [...subscriptions, { id: Date.now(), ...newSub }];
    setSubscriptions(updated);
    setNewSub({ name: '', cost: '', url: '' });
    if(user) saveDashboard(user.uid, memo, habits, commandLinks, updated);
  };
  ...
  const toggleHabit = (id) => {
    ...
    setHabits(updated);
    if(user) saveDashboard(user.uid, memo, updated, commandLinks, subscriptions);
  };
  ...
  const getGreetingIcon = () => {
    ...
    if (h >= 5 && h < 12) return <Sunrise className="w-6 h-6 text-yellow-400 animate-bounce" />;
    if (h >= 12 && h < 17) return <Sun className="w-6 h-6 text-orange-400 animate-spin" style={{animationDuration:'10s'}}/>;
    return <Moon className="w-6 h-6 text-indigo-400 animate-pulse" />;
  };
        ...
        <aside className={`fixed inset-y-0 left-0 z-50 bg-pro-bg/80 backdrop-blur-xl border-r border-pro-border flex flex-col justify-between py-6 transition-all duration-300 ease-in-out shadow-2xl ${isSidebarOpen ? 'w-64' : 'w-20'}`} onMouseEnter={() => setSidebarOpen(true)} onMouseLeave={() => setSidebarOpen(false)}>
          <div>
            <div className="flex items-center gap-3 mb-10 px-4 h-12">
              <div className="w-12 h-12 shrink-0 bg-gradient-primary rounded-xl flex items-center justify-center text-white font-bold text-xl shadow-lg cursor-pointer">{isSidebarOpen ? 'L' : <Menu className="w-6 h-6" />}</div>
              <h1 className={`text-2xl font-bold text-pro-white tracking-tight transition-opacity duration-300 ${isSidebarOpen ? 'opacity-100' : 'opacity-0 hidden'}`}>Lumina</h1>
            </div>
            <nav className="space-y-2 px-2">
              {[{id:'dashboard',icon:LayoutDashboard,label:'Dashboard'},{id:'write',icon:PenTool,label:'Journal'},{id:'entries',icon:BookOpen,label:'Entries'},{id:'stats',icon:BarChart2,label:'Analytics'}].map(item => (
                <button key={item.id} onClick={() => handleNavigation(item.id)} className={`flex items-center h-12 w-full rounded-xl transition-all duration-200 group relative ${view === item.id ? 'bg-pro-card text-pro-primary shadow-sm border border-pro-border' : 'text-gray-500 hover:bg-white/5 hover:text-pro-white'}`}>
                  <div className="w-16 h-12 flex items-center justify-center shrink-0 relative">
                    ...
                    {securityPin && (item.id === 'write' || item.id === 'entries') && <div className="absolute top-2 right-4 w-2 h-2 bg-purple-500 rounded-full"></div>}
                  </div>
                  <span className={`font-medium whitespace-nowrap overflow-hidden transition-all duration-300 ${isSidebarOpen ? 'opacity-100 w-auto' : 'opacity-0 w-0'}`}>{item.label}</span>
                </button>
              ))}
            </nav>
          </div>
          <div className="border-t border-pro-border pt-6 px-2 space-y-2">
             <button onClick={() => setIsSettingsOpen(true)} className="flex items-center h-12 w-full rounded-xl text-gray-500 hover:bg-white/5 hover:text-pro-white transition-colors"><div className="w-16 h-12 flex items-center justify-center shrink-0"><SettingsIcon className="w-5 h-5" /></div><span className={`font-medium whitespace-nowrap overflow-hidden transition-all duration-300 ${isSidebarOpen ? 'opacity-100 w-auto' : 'opacity-0 w-0'}`}>Settings</span></button>
        ...
        {!zenMode && (
          <header className="sticky top-0 z-40 bg-pro-bg/80 backdrop-blur-md px-8 py-6 flex justify-between items-center">
            <div className="flex items-center gap-4">
              {getGreetingIcon()}
              <div><h2 className="text-2xl font-bold text-pro-white">{time.getHours() < 12 ? 'Good Morning,' : 'Good Evening,'}<span className="text-pro-primary"> Achintha</span></h2><p className="text-sm text-gray-500 mt-1">Ready to organize your thoughts?</p></div>
            </div>
            <div className="flex items-center gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="lg:col-span-2 bg-pro-card rounded-2xl p-6 border border-pro-border shadow-sm flex flex-col justify-between relative overflow-hidden group">
                <div className="flex justify-between items-start z-10">
                  ...
                  <div className="text-right"><span className="text-3xl font-bold text-pro-white">{weather?.temperature}°</span><p className="text-xs text-gray-500 uppercase tracking-wider">Colombo, LK</p></div>
                </div>
                <div className="mt-8 z-10">
                  <h3 className="text-6xl font-bold text-pro-white tracking-tighter font-mono">{time.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</h3>
                  <p className="text-pro-text mt-2 font-medium">{time.toLocaleDateString(undefined, {weekday:'long', month:'long', day:'numeric'})}</p>
                </div>
              </div>
              ...
              <div className="lg:col-span-1 bg-gradient-primary rounded-2xl p-6 shadow-lg shadow-indigo-500/20 text-white flex flex-col justify-between relative overflow-hidden">
                <div className="z-10 w-full">
                  <div className="flex justify-between items-center mb-1">
                     ...
                     <button onClick={() => setIsEditingTimer(true)} className="text-indigo-200 hover:text-white p-1 rounded hover:bg-white/10"><Edit3 className="w-4 h-4" /></button>
                  </div>
                  {isEditingTimer ? (
                    <div className="flex items-center gap-2 mb-2">
                      <input type="number" autoFocus value={customMinutes} onChange={(e) => setCustomMinutes(e.target.value)} className="w-16 bg-white/20 text-white text-2xl font-bold font-mono p-1 rounded border border-white/30 text-center" />
                      <button onClick={() => { setPomoTime(customMinutes*60); setInitialPomoTime(customMinutes*60); setIsEditingTimer(false); }} className="bg-white/20 hover:bg-white/30 p-1 px-3 rounded text-sm">Set</button>
                    </div>
                  ) : (
                    <span className="text-5xl font-bold font-mono block cursor-pointer">{Math.floor(pomoTime/60).toString().padStart(2,'0')}:{ (pomoTime%60).toString().padStart(2,'0') }</span>
                  )}
                </div>
                <div className="flex gap-3 mt-4 z-10">
                  <button onClick={() => setPomoActive(!pomoActive)} className="flex-1 bg-white/20 hover:bg-white/30 backdrop-blur-sm py-2 rounded-lg font-medium transition-colors flex items-center justify-center gap-2">{pomoActive ? <Pause className="w-4 h-4"/> : <Play className="w-4 h-4"/>} {pomoActive ? 'Pause' : 'Start'}</button>
                  <button onClick={() => {setPomoActive(false); setPomoTime(initialPomoTime)}} className="px-3 bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-lg"><RotateCw className="w-4 h-4"/></button>
                </div>
              </div>
                ...
                <div className="flex-1 overflow-y-auto space-y-2 max-h-40 custom-scrollbar pr-2">
                  {habits.map(h => (
                    <div key={h.id} className="group flex items-center justify-between p-3 rounded-xl bg-pro-bg border border-pro-border hover:border-pro-primary/50 transition-colors cursor-pointer" onClick={() => toggleHabit(h.id)}>
                      <div className="flex items-center gap-3"><div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${h.completed ? 'bg-pro-primary border-pro-primary' : 'border-gray-600'}`}>{h.completed && <CheckCircle className="w-3 h-3 text-white" />}</div><span className={`text-xs ${h.completed ? 'text-gray-500 line-through' : 'text-gray-200'}`}>{h.label}</span></div>
              ...
              <div className="lg:col-span-1 bg-pro-card rounded-2xl p-6 border border-pro-border shadow-sm flex flex-col">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="flex gap-2">
                    <span className="text-xs bg-pro-bg px-2 py-1 rounded text-green-400">${getTotalSubscriptionCost()}</span>
                    <button onClick={() => setIsEditingSubs(!isEditingSubs)} className="text-gray-500 hover:text-white"><Edit3 className="w-3 h-3"/></button>
                  </div>
                </div>
                {isEditingSubs ? (
                  <div className="space-y-2 animate-fadeIn">
                    <div className="flex gap-1"><input className="flex-1 bg-pro-bg border border-pro-border rounded px-2 py-1 text-xs text-white" placeholder="Name" value={newSub.name} onChange={e=>setNewSub({...newSub, name:e.target.value})} /><input className="w-16 bg-pro-bg border border-pro-border rounded px-2 py-1 text-xs text-white" placeholder="$" value={newSub.cost} onChange={e=>setNewSub({...newSub, cost:e.target.value})} /></div>
                    <div className="flex gap-1"><input className="flex-1 bg-pro-bg border border-pro-border rounded px-2 py-1 text-xs text-white" placeholder="URL (for icon)" value={newSub.url} onChange={e=>setNewSub({...newSub, url:e.target.value})} /><button onClick={addSubscription} className="bg-orange-600 text-white px-2 rounded"><Plus className="w-3 h-3"/></button></div>
                    <div className="h-24 overflow-y-auto mt-2 space-y-1">{subscriptions.map(s => (<div key={s.id} className="flex justify-between items-center text-xs p-1 bg-pro-bg rounded border border-pro-border"><span>{s.name}</span><button onClick={() => removeSubscription(s.id)} className="text-red-400"><X className="w-3 h-3"/></button></div>))}</div>
                  </div>
                ) : (
                  <div className="flex-1 space-y-2 overflow-y-auto max-h-48 custom-scrollbar">
                    {subscriptions.map(s => (
                      <a key={s.id} href={s.url} target="_blank" rel="noreferrer" className="flex items-center justify-between p-2 bg-pro-bg border border-pro-border rounded-lg hover:border-orange-500/50 transition-colors group">
                        <div className="flex items-center gap-3">
                          <img src={getFavicon(s.url)} alt="" className="w-6 h-6 rounded-full bg-white p-0.5" onError={(e) => e.target.src = 'https://via.placeholder.com/24'} />
                          <span className="text-sm font-medium text-gray-300 group-hover:text-white">{s.name}</span>
                        </div>
                        <span className="text-xs font-mono text-gray-500">${s.cost}</span>
                      </a>
                    ))}
                    {subscriptions.length === 0 && <p className="text-xs text-center text-gray-600 mt-4">No active subscriptions.</p>}
                  </div>
                )}
              </div>
              ...
              <div className="lg:col-span-1 bg-pro-card rounded-2xl p-6 border border-pro-border shadow-sm flex flex-col justify-between relative overflow-hidden group hover:border-blue-500/50 transition-colors">
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-sm text-gray-400 mb-4 line-clamp-2">Access your learning dashboard.</p>
                </div>
              ...
              <div className="lg:col-span-1 bg-pro-card rounded-2xl p-6 border border-pro-border shadow-sm flex flex-col">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button onClick={() => setIsEditingCommands(!isEditingCommands)} className="text-gray-500 hover:text-white"><Edit3 className="w-3 h-3"/></button>
                </div>
                {isEditingCommands ? (
                  <div className="space-y-2 animate-fadeIn">
                  ...
                  <div className="flex-1 grid grid-cols-2 gap-2 overflow-y-auto custom-scrollbar content-start">
                    {commandLinks.map(link => (
                      <a key={link.id} href={link.url} target="_blank" rel="noreferrer" className="flex flex-col items-center justify-center p-3 bg-pro-bg border border-pro-border rounded-xl hover:border-purple-500 transition-all group h-20">
                        <img src={getFavicon(link.url)} alt="" className="w-6 h-6 mb-2 rounded bg-white p-0.5" onError={(e) => e.target.src = 'https://via.placeholder.com/24'} />
                        <span className="text-[10px] text-gray-400 text-center truncate w-full">{link.title}</span>
                      </a>
                    ))}
                    {commandLinks.length === 0 && <p className="col-span-2 text-center text-xs text-gray-600 py-4">Add links via Edit mode.</p>}
                  </div>
                )}
              </div>

              {/* 10. Quick Notes (2 cols) */}
              <div className="lg:col-span-2 bg-pro-card rounded-2xl p-6 border border-pro-border shadow-sm flex flex-col">
                <div className="flex justify-between items-center mb-2"><h4 className="font-semibold text-pro-white flex items-center gap-2"><Save className="w-4 h-4 text-yellow-500"/> Quick Notes</h4><span className="text-[10px] text-gray-600 uppercase">Auto-Sync</span></div>
                <textarea value={memo} onChange={handleMemo} placeholder="Capture your thoughts..." className="flex-1 w-full bg-pro-bg rounded-xl border border-pro-border p-4 text-sm text-gray-300 focus:outline-none focus:border-pro-primary resize-none transition-colors" style={{minHeight: '140px'}} />
              </div>
              ...
              <div className="bg-pro-card border border-pro-border rounded-2xl p-8 w-full max-w-sm shadow-2xl flex flex-col items-center">
                ...
                <div className="grid grid-cols-3 gap-3 w-full max-w-[240px]">
                  {[1, 2, 3, 4, 5, 6, 7, 8, 9, 'C', 0, 'Go'].map((key) => (
                    <button key={key} onClick={() => {if (key === 'C') setPinInput(''); else if (key === 'Go') handlePinSubmit(); else if (pinInput.length < 6) setPinInput(prev => prev + key);}} className={`h-12 rounded-xl text-lg font-bold transition-all active:scale-95 flex items-center justify-center ${key === 'Go' ? 'bg-purple-600 text-white' : key === 'C' ? 'bg-red-500/10 text-red-400' : 'bg-pro-bg border border-pro-border text-gray-300 hover:bg-white/5'}`}>{key === 'Go' ? <Unlock className="w-5 h-5"/> : key}</button>
                  ))}
                </div>
                <button onClick={() => { setIsPinPromptOpen(false); if(pinMode === 'verify_remove') setIsSettingsOpen(true); }} className="mt-6 text-xs text-gray-500 hover:text-white">Cancel</button>
              </div>
            </div>
          )}
          ...
          {isSettingsOpen && (
            <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fadeIn">
              <div className="bg-pro-card border border-pro-border rounded-2xl p-6 w-full max-w-md shadow-2xl relative">
                ...
                <h3 className="text-xl font-bold text-pro-white mb-6 flex items-center gap-2"><SettingsIcon className="w-5 h-5 text-gray-400" /> Settings</h3>
                <div className="space-y-4">
                  <div className="bg-pro-bg rounded-xl border border-pro-border p-4">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-3"><div className={`p-2 rounded-lg ${securityPin ? 'bg-green-500/10 text-green-500' : 'bg-gray-700/30 text-gray-500'}`}><Shield className="w-5 h-5" /></div><div><h4 className="font-medium text-white">Journal Security</h4><p className="text-xs text-gray-500">{securityPin ? 'Active' : 'Disabled'}</p></div></div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 mt-4">{securityPin ? (<><button onClick={() => { setPinMode('verify_current'); setPinInput(''); setIsPinPromptOpen(true); setIsSettingsOpen(false); }} className="py-2 px-3 bg-pro-card border border-pro-border rounded-lg text-xs text-white hover:bg-white/5">Change PIN</button><button onClick={initiateRemovePin} className="py-2 px-3 bg-red-500/10 border border-red-500/20 rounded-lg text-xs text-red-400 hover:bg-red-500/20">Remove</button></>) : (<button onClick={() => { setPinMode('setup'); setPinInput(''); setIsPinPromptOpen(true); setIsSettingsOpen(false); }} className="col-span-2 py-2 px-3 bg-blue-600 hover:bg-blue-500 rounded-lg text-xs text-white font-medium flex items-center justify-center gap-2"><KeyRound className="w-3 h-3"/> Setup PIN</button>)}</div>
                  </div>
                </div>
              </div>
            </div>
